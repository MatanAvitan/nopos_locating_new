...
\caption{LayerNorm linearization effect. \textbf{Left}: Before LN (post-attention), position is encoded in directional structure ($R^2=0.80$) but full activations yield poor linear decodability ($R^2=0.04$). \textbf{Right}: After LN, position becomes linearly decodable from both direction and norm ($R^2 \approx 0.76$). LN transforms complex directional encoding into simple norm-based signal.}
