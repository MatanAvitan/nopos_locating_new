...
We trace how positional information flows through the network by analyzing activations at each component. A key insight is that positional information undergoes a \textbf{three-phase transformation}: initially there is no signal at the input, then position becomes encoded in the \textit{variance} of activations after attention, and finally position is encoded in the \textit{mean} of activations after LayerNorm.
...
LayerNorm computes the standard deviation $\sigma_i$ across channels at each position, effectively reading out the variance signal. Since $\sigma_i$ varies monotonically with position, the scale factor $1/(\sigma_i + \epsilon)$ carries positional information. After normalization, positional information is encoded in the population-level \textbf{mean} of activations, completing the variance-to-mean transformation. While individual samples show normalized activations with zero mean, averaging across many samples reveals smooth monotonic patterns. Figure~\ref{fig:layernorm-paradox} visualizes these population-level patterns across 30K samples.
...
A fundamental question arises: is position encoded in the \textit{direction} (shape) of activation vectors or their \textit{magnitude} (norm)? We decompose activations as $h = \|h\| \cdot \hat{h}$ where $\hat{h} = h/\|h\|$ is the unit direction, and train separate linear probes on each component.
...
\midrule
post\_attn & 0.04 & 0.51 & \textbf{0.80} \\
post\_ln2 & 0.76 & 0.76 & 0.75 \\
post\_mlp & 0.70 & 0.37 & \textbf{0.75} \\
\bottomrule
\end{tabular}
\caption{Position decoding $R^2$ from full activations, norm only, and direction only at different layers. Direction dominates before LayerNorm; both become equally informative after LN.}
\label{tab:direction-norm}
\end{table}

Table~\ref{tab:direction-norm} reveals a striking pattern: \textbf{before LayerNorm}, position is encoded primarily in direction ($R^2=0.80$) rather than norm ($R^2=0.51$), and full activations yield poor linear decodability ($R^2=0.04$). This occurs because averaging different numbers of embedding vectors creates complex, high-dimensional structure that is not easily captured by a linear probe.

\textbf{After LayerNorm}, linear decodability dramatically improves ($R^2=0.76$), with both direction and norm becoming equally informative. This improvement is not because LayerNorm ``amplifies'' the position signal, but because it \textit{transforms} the complex directional encoding into a simpler, more linear form.

\paragraph{Why LayerNorm Linearizes the Position Signal.}
The key insight is that LayerNorm's normalization step converts the nonlinear relationship between position and activation structure into a near-linear relationship in the output norm. Consider the pre-LN activation $h_i$ at position $i$, which averages $i+1$ embedding vectors. The input variance $\sigma_i^2 \propto 1/(i+1)$ follows a hyperbolic decay. LayerNorm computes:
\begin{equation}
\text{LN}(h_i) = \gamma \cdot \frac{h_i - \mu_i}{\sigma_i} + \beta
\end{equation}
where $\mu_i$ and $\sigma_i$ are the mean and standard deviation across dimensions. The output norm becomes:
\begin{equation}
\|\text{LN}(h_i)\| = \|\gamma\| \cdot \frac{\|h_i - \mu_i\|}{\sigma_i} \approx \|\gamma\| \cdot \sqrt{d} \cdot \sqrt{1 + \epsilon_i}
\end{equation}
where $d$ is the embedding dimension and $\epsilon_i$ captures finite-sample deviations.

\paragraph{The Bessel Correction Effect.}
A subtle but important factor is the Bessel correction in variance estimation. PyTorch's LayerNorm uses the biased estimator (dividing by $n$), but when we measure the output variance, we observe:
\begin{equation}
\text{Var}_{\text{measured}}[\text{LN}(h_i)] = \frac{d}{d-1} \cdot \text{Var}_{\text{LN}}[h_i] = \frac{d}{d-1}
\end{equation}
This creates a constant offset. However, the \textit{input} to LayerNorm has position-dependent variance $\sigma_i^2 \propto 1/(i+1)$. The normalization process encodes this position-dependent input variance into tiny but highly systematic deviations in the output norm. Specifically, positions with higher input variance (early positions) produce outputs with slightly larger norms than positions with lower input variance (later positions).

The correlation between output variance and position increases from $r=-0.58$ pre-LN to $r=-0.96$ post-LN. This is not amplification---the absolute variance differences shrink dramatically (from range $[0.001, 0.025]$ to $[0.997, 1.003]$). Rather, the normalization process creates a \textit{monotonic, near-linear} mapping from input variance to output norm, making the position signal trivially decodable by a linear probe.

\paragraph{Information Flow Through the Transformer Block.}
Critically, \textbf{after the MLP}, direction again dominates ($R^2=0.75$ vs $0.37$), suggesting the MLP redistributes positional information back into directional components. This reveals a complete picture of information flow: position enters as directional structure (post-attention), is linearized into norm by LayerNorm, and is then re-encoded into directions by the MLP for downstream processing.
