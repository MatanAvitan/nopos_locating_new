...
\paragraph{Causal Intervention: Norm vs Direction.}
Despite the high norm-only $R^2$ values, causal intervention experiments reveal that \textbf{norm is not causally used for position decoding}. We perform three interventions on post-LN2 activations:

\begin{enumerate}
    \item \textbf{Norm equalization}: Set all positions to the same norm while preserving directions
    \item \textbf{Norm shuffle}: Randomly permute norms across positions
    \item \textbf{Direction shuffle}: Keep norms fixed, randomize directions
\end{enumerate}

The results are striking: norm equalization causes \textit{no drop} in position decoding $R^2$ (0.14 $\rightarrow$ 0.14 for synthetic, 0.46 $\rightarrow$ 0.46 for trained), while direction shuffle \textit{completely destroys} it ($R^2 \rightarrow -0.14$ and $-0.47$ respectively). This demonstrates that while norm strongly \textit{correlates} with position (a probe on norm alone achieves $R^2=0.96$), the information is redundantly encoded in direction---a linear probe on full activations learns to use direction, not norm. The norm-position correlation is thus an \textit{epiphenomenon} of the underlying directional encoding, not a functionally utilized signal.
...
\midrule
\multicolumn{3}{l}{\textit{Causal interventions}} \\
Sec.~5.2 Norm vs Dir & All configs & \texttt{post\_ln2\_norm\_causal\_intervention.py} \\
\midrule
...
\paragraph{Causal Intervention (Section~5.2).}
Values from \texttt{post\_ln2\_norm\_intervention\_results.json}. The experiment tests whether norm is \textit{causally} used by comparing full probe $R^2$ before and after intervention:
\begin{itemize}
    \item \textbf{Synthetic-Large}: Baseline full $R^2=0.14$, after norm equalization $R^2=0.14$ (0\% drop), after direction shuffle $R^2=-0.14$ (destroyed)
    \item \textbf{Synthetic-Small}: Baseline full $R^2=0.18$, after norm equalization $R^2=0.18$ (0\% drop), after direction shuffle $R^2=-0.18$ (destroyed)
    \item \textbf{Trained}: Baseline full $R^2=0.46$, after norm equalization $R^2=0.46$ (0\% drop), after direction shuffle $R^2=-0.47$ (destroyed)
\end{itemize}
Conclusion: Norm correlates with position but is not causally used; direction is the functional signal.
