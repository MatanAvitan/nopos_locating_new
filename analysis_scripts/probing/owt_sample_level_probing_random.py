"""
OWT Sample-Level Position Probing Analysis - RANDOM INITIALIZATION

Trains linear probes on each component (layer) of all 4 OWT architectures
using RANDOMLY INITIALIZED models (not trained) to understand how position
is encoded at initialization.

This shows the "inductive bias" for position encoding before any training.
"""
...
def create_layerwise_figure(
    all_results: Dict, output_path: Path, title_suffix: str = ""
):
    ...
    for ax_idx, (metric, title) in enumerate(zip(metrics, titles)):
        ...
        for exp_name, data in all_results.items():
            ...
            y_values = [random_data[layer][metric] for layer in LAYERS]
...
def create_comparison_heatmap(all_results: Dict, output_path: Path):
    ...
    for ax_idx, (metric, title) in enumerate(zip(metrics, titles)):
        ...
        for i, exp_name in enumerate(exp_names):
            if "random" in all_results[exp_name]:
                random_data = all_results[exp_name]["random"]
                for j, layer in enumerate(LAYERS):
                    matrix[i, j] = random_data[layer][metric]
...
def main():
    print("=" * 70)
    print("OWT SAMPLE-LEVEL POSITION PROBING - RANDOM INITIALIZATION")
    print("=" * 70)
    print(f"\nDevice: {DEVICE}")
    ...
    for exp in EXPERIMENTS:
        print(f"\n{'#' * 70}")
        print(f"# Experiment: {exp.name} (RANDOM INIT)")
        print(f"{'#' * 70}")

        # Create randomly initialized model
        random_model, config = create_random_model(exp)
        print(f"\nCreated random model with config matching: {exp.checkpoint_path}")

        # Analyze random model
    ...
    # Save results
    output_path = RESULTS_DIR / "owt_sample_level_probing_random_results.json"
    ...
    create_layerwise_figure(
        all_results, PLOTS_DIR / "owt_sample_level_layerwise_random"
    )

    # Heatmap comparison
    create_comparison_heatmap(
        all_results, PLOTS_DIR / "owt_sample_level_heatmap_random"
    )

    # Print final summary table
    print("\n" + "=" * 90)
    print("FINAL SUMMARY: Sample-Level Position Probing R² by Layer (RANDOM INIT)")
    print("=" * 90)
    ...
    for name, data in all_results.items():
        if "random" in data:
            row = f"{name:<20}"
            for layer in LAYERS:
                row += f" {data['random'][layer]['full_r2']:>12.3f}"
            print(row)
    ...
    for name, data in all_results.items():
        if "random" in data:
            row = f"{name:<20}"
            for layer in LAYERS:
                row += f" {data['random'][layer]['direction_r2']:>12.3f}"
            print(row)
    ...
    for name, data in all_results.items():
        if "random" in data:
            row = f"{name:<20}"
            for layer in LAYERS:
                row += f" {data['random'][layer]['norm_r2']:>12.3f}"
            print(row)
