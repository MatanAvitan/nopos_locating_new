...
def get_batch_with_bos(data, batch_size: int, block_size: int, bos_position: int, device: str):
...
def create_attention_heatmap(attn_weights, block_name: str, head_idx: int,
                             bos_position: int = 80, block_size: int = 128):
    ...
    ax.annotate('', xy=(bos_position, -1), xytext=(bos_position, -6),
                arrowprops=dict(arrowstyle='->', color='#C44E52', lw=2),
                annotation_clip=False)
    ax.text(bos_position, -8, 'BOS', ha='center', va='top', fontsize=8,
            color='#C44E52', fontweight='bold', clip_on=False)

    # Left margin - mark query position
    ax.annotate('', xy=(-1, bos_position), xytext=(-6, bos_position),
                arrowprops=dict(arrowstyle='->', color='#C44E52', lw=2),
                annotation_clip=False)
    ax.text(-8, bos_position, 'BOS', ha='right', va='center', fontsize=8,
            color='#C44E52', fontweight='bold', clip_on=False)
...
def create_bos_attention_summary(attn_weights, block_name: str,
                                  bos_position: int = 80, n_heads: int = 12):
    ...
    bars = ax.bar(range(n_heads), attn_to_bos, color='#4C72B0', edgecolor='black', linewidth=0.5)
    ...
    ax.set_title(f'{block_name}: Attention to BOS Position ({bos_position})', fontweight='bold')
...
def create_full_attention_grid(attn_weights, block_name: str, bos_position: int = 80):
    ...
    for h in range(n_heads):
        ...
        # Add small triangle markers on margins instead of lines
        # Top margin marker for key position
        ax.plot(bos_position, -3, marker='v', color='#C44E52', markersize=5,
                clip_on=False, markeredgecolor='black', markeredgewidth=0.3)
        # Left margin marker for query position
        ax.plot(-3, bos_position, marker='>', color='#C44E52', markersize=5,
                clip_on=False, markeredgecolor='black', markeredgewidth=0.3)
    ...
    fig.suptitle(f'{block_name} Attention Maps (BOS @ pos {bos_position})',
...
def main():
    ...
    output_dir = '/home/nlp/matan_avitan/git/nopos_locating_new/results/attention_maps_bos80'
    ...
    np.random.seed(42)
    x = get_batch_with_bos(data, batch_size=64, block_size=128,
                           bos_position=BOS_POSITION, device=device)
    ...
    fig1_grid.savefig(os.path.join(output_dir, 'block1_attention_grid.png'), dpi=150, bbox_inches='tight')
    fig1_grid.savefig(os.path.join(output_dir, 'block1_attention_grid.pdf'), bbox_inches='tight')
    plt.close(fig1_grid)
    ...
    fig2_grid.savefig(os.path.join(output_dir, 'block2_attention_grid.png'), dpi=150, bbox_inches='tight')
    fig2_grid.savefig(os.path.join(output_dir, 'block2_attention_grid.pdf'), bbox_inches='tight')
    plt.close(fig2_grid)

    # BOS attention summary
    fig1_bos, attn1_to_bos = create_bos_attention_summary(attn1, "Block 1", BOS_POSITION)
    wandb.log({"attention/block1_bos_attention": wandb.Image(fig1_bos)})
    fig1_bos.savefig(os.path.join(output_dir, 'block1_bos_attention.png'), dpi=150, bbox_inches='tight')
    plt.close(fig1_bos)

    fig2_bos, attn2_to_bos = create_bos_attention_summary(attn2, "Block 2", BOS_POSITION)
    wandb.log({"attention/block2_bos_attention": wandb.Image(fig2_bos)})
    fig2_bos.savefig(os.path.join(output_dir, 'block2_bos_attention.png'), dpi=150, bbox_inches='tight')
    plt.close(fig2_bos)
    ...
    for h, val in enumerate(attn2_to_bos):
        if val > 0.05:
            ...
            fig_head.savefig(os.path.join(output_dir, f'block2_head{h}_detail.png'), dpi=150, bbox_inches='tight')
            plt.close(fig_head)
    ...
    wandb.run.summary["block1_bos_heads"] = [i for i, v in enumerate(attn1_to_bos) if v > 0.1]
    wandb.run.summary["block2_bos_heads"] = [i for i, v in enumerate(attn2_to_bos) if v > 0.1]

    print("\n" + "=" * 50)
    print("BOS Attention Analysis (BOS @ position 80)")
    print("=" * 50)
    print(f"Block 1 - Max BOS attention: {max(attn1_to_bos):.4f}")
    print(f"Block 1 - BOS heads (>0.1): {[i for i, v in enumerate(attn1_to_bos) if v > 0.1]}")
    print(f"Block 2 - Max BOS attention: {max(attn2_to_bos):.4f}")
    print(f"Block 2 - BOS heads (>0.1): {[i for i, v in enumerate(attn2_to_bos) if v > 0.1]}")
    print("=" * 50)
